...
Point.GENERATOR = Point.from_ints(Gx, Gy)

assert Point.ZERO + Point.ZERO == Point.ZERO

JUBJUB_MONTGOMERY_A = 168698
JUBJUB_MONTGOMERY_B = 1


def x_to_montgomery_y(x, sign):
  tmp = (x*x*x + JUBJUB_MONTGOMERY_A*x*x + x) % q_j
  ret = mod_sqrt(tmp, q_j)
  if ret:
    return ret[1] if sign else ret[0]
  return None
  

def montgomery_to_twisted_edwards(x, y):
  u = x * mod_inv(y, q_j)
  v = (x-1) * mod_inv(x+1, q_j)
  return u % q_j, v % q_j
  

#TODO ask stackexchange why this is bad vs Elligator 2 or something
def hash_to_curve(seed, n):
  nonce = 0
  ret = []
  while len(ret) < n:
    ...
    if not x < q_j:
      continue
    y = x_to_montgomery_y(x, sign)
    if y:
      u, v = montgomery_to_twisted_edwards(x, y)
      P = Point.from_ints(u, v) * Fq(8)
      ret.append(P)
  return ret
